...
// on page load, insert navber.html into header using jQuery
...
function showProducts(response) {
  ...
  for (let index in products) {
    //checks for every third index, inserts a new row if so
    if (index % 3 ===0) {
      html += '<div class="row">';
    }
    ...
    html += `
      <div class="col-md-4">
        <div class="card">
          <div class="card-img-top"><img class="card-img" src="http://placehold.it/250x250" alt="Placeholder"></div>
...
function removeItem(id) {
  ...
  //loop through cart and check id passed in against each elements
...
function showCart() {
  ...
  //Loop through cart and add to html vaiablr for each row
  for (let index in cart) {
    html += `
      <tr>
        <td>1</td>
